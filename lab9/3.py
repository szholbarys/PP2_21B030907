...
# border
pygame.draw.line(screen,color_set['black'],(0,HEIGHT-50),(WIDTH,HEIGHT-50),1)

# all info about shapes
erase = pygame.transform.scale(pygame.image.load(os.path.join('images','eraser.png')),(40,40)) # range: 0-40
...
pygame.draw.rect(screen,color_set['black'],(210,HEIGHT-40,40,40),3) # range: 210 - 250

pygame.draw.polygon(screen,color_set['black'],([260,HEIGHT],[280,HEIGHT-40],[300,HEIGHT]),3) # range: 260-300

pygame.draw.polygon(screen,color_set['black'],([310,HEIGHT-20],[330,HEIGHT-40],[350,HEIGHT-20],[330,HEIGHT]),3) # range: 310 - 350

# width of the line
plus = pygame.transform.scale(pygame.image.load(os.path.join('images','plus.png')),(40,40))
screen.blit(plus,(WIDTH/2-80,HEIGHT-40)) 

minus = pygame.transform.scale(pygame.image.load(os.path.join('images','minus.png')),(40,40))
screen.blit(minus,(WIDTH/2-40,HEIGHT-40))
   ...
   for event in pygame.event.get():
      ...
      if event.type == pygame.MOUSEBUTTONDOWN:
         ...
         if pos_select[1] > HEIGHT-50:
            ...
            if 160 < pos_select[0] < 200:
               draw = 'circle'
            if 210 < pos_select[0] < 250:
               draw = 'square'
            if 260 < pos_select[0] < 300:
               draw = 'triangle'
            if 310 < pos_select[0] < 350:
               draw = 'rhombus'
            # selecting colors
      ...
      if draw == 'pen':
         ...
         if event.type == pygame.MOUSEMOTION:
            ...
            if prev:
               if prev[1]<HEIGHT-50 and cur[1]<HEIGHT-50:
      ...
      if draw == 'square':
         ...
         if event.type == pygame.MOUSEBUTTONUP:
            ...
            if prev[1] < HEIGHT-50 and cur[1] < HEIGHT-50:
               ...
               if cur[1] - y < 0:
                  y -= edge
               pygame.draw.rect(screen,color,(x,y,edge,edge),line_width)

      # selecting the triangle
      if draw == 'triangle':
         if event.type == pygame.MOUSEBUTTONDOWN:
            prev = pos_select
         if event.type == pygame.MOUSEBUTTONUP:
            ...
            if prev[1] < HEIGHT-50 and cur[1] < HEIGHT-50:
               x,y = prev[0], prev[1]
               edge = cur[0]-prev[0]
               pygame.draw.polygon(screen,color,([x,y+edge],[x+edge/2,y],[x+edge,y+edge]),line_width)
      
      # selecting the rhombus
      if draw == 'rhombus':
         if event.type == pygame.MOUSEBUTTONDOWN:
            prev = pos_select
         if event.type == pygame.MOUSEBUTTONUP:
            ...
            if prev[1] < HEIGHT-50 and cur[1] < HEIGHT-50:
               x,y = prev[0], prev[1]
               edge = max(cur[0]-prev[0], cur[1]-prev[1])
               pygame.draw.polygon(screen,color,([x,y+edge/2],[x+edge/4,y],[x+edge/2,y+edge/2],[x+edge/4,y+edge]),line_width)
      ...
      if draw == 'eraser':
         ...
         if event.type == pygame.MOUSEMOTION:
            cur = pygame.mouse.get_pos()
            if prev:
              if prev[1]<HEIGHT-70 and cur[1]<HEIGHT-70:
